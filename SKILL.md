...
- **`help [<커맨드명>]`** — 사용 가능한 커맨드 전체 목록을 보여준다. 이 장(§2)의 목록을
  그대로 재사용해서 보여준다 — 별도로 요약본을 만들어 유지하지 않는다(두 곳에 같은 정보를
  들고 있으면 드리프트되므로, 여기서도 SoT를 하나로 유지한다). 커맨드명을 붙이면
  (`help draft`처럼) 해당 커맨드가 링크한 상세 장의 핵심만 요약해서 보여준다. 이 스킬이
  처음이거나 뭐가 있었는지 헷갈릴 때 언제든 불러도 된다 — 조회 전용이라 부작용이 없다.
